...
void JADE::run() {
  ...
  // for the parameter adaptation method in JADE 
  double dist_location_sf = 0.5; //scaling_factor;
  double dist_mean_cr = 0.5; //cross_rate;
  ...
  while (nfes < max_num_evaluations && error_val > 0) {  
    ...
    // for the parameter adaptation method in JADE
    std::normal_distribution<double> randNormalCR(dist_mean_cr, 0.1);
    std::cauchy_distribution<double> randCauchySF(dist_location_sf, 0.1);
    ...
    if (success_param_id > 0) {
      ...
      tmp_val = 0;
      for (int i = 0; i < success_param_id; i++) tmp_val += success_cr_vals[i];
      success_mean_cr = tmp_val / success_param_id;      
      dist_mean_cr = (1.0 - jade_learning_rate) * dist_mean_cr + jade_learning_rate * success_mean_cr;
	...
	tmp_val2 += success_sf_vals[i];
      ...
      dist_location_sf = (1.0 - jade_learning_rate) * dist_location_sf + jade_learning_rate * success_mean_sf;
    }   
